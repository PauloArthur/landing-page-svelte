...
<div class={`relative w-full ${carouselName}`}>
	...
	{#if showNavigation}
		<div
			class="absolute left-0 flex items-center justify-between w-full h-0 px-2 lg:px-4 top-1/2"
			data-glide-el="controls"
		>
			<button
				on:click={triggerLoad}
				class="inline-flex items-center justify-center w-10 h-10 lg:w-20 lg:h-20"
				data-glide-dir="<"
	...
	{#if showIndicators}
		<div
			class={`${indicatorClasses} flex items-center justify-center w-full gap-2`}
